...
class _ChatInputBarState extends State<ChatInputBar>
  ...
  static const MethodChannel _speechIntentChannel = MethodChannel('app.speech_intent');
  bool _speechAvailable = false;
  bool _speechInitializing = false;
  bool _speechListening = false;
  bool _speechFallbackInProgress = false;
      ...
      final available = await _speech.initialize(
        ...
        onError: (error) {
          ...
          if (Platform.isAndroid &&
              (errorMsg == 'error_permission' ||
                  errorMsg == 'error_client' ||
                  errorMsg == 'error_audio_error')) {
            unawaited(_startSystemSpeechDialog(
              reason: '系统语音服务不可用，已切换到系统弹窗识别',
            ));
            return;
          }
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('语音识别失败：$errorMsg')),
          );
        },
      );
  ...
  Future<void> _toggleSpeechRecognition() async {
    ...
    if (!ready || !mounted) {
      if (Platform.isAndroid) {
        await _startSystemSpeechDialog();
      }
      return;
    }
  ...
  Future<void> _startSystemSpeechDialog({String? reason}) async {
    if (_speechFallbackInProgress || _composerLocked || !mounted) return;
    _speechFallbackInProgress = true;
    _speechBaseText = _controller.text;
    try {
      widget.focusNode?.unfocus();
      ...
      final text = await _speechIntentChannel.invokeMethod<String>(
        'recognize',
        <String, Object?>{
          'localeId': _speechLanguage.localeId,
          'prompt': '请开始说话',
      ...
      if (text != null && text.trim().isNotEmpty) {
        _insertRecognizedSpeech(text);
      }
    } on PlatformException catch (e) {
      if (!mounted) return;
      final message = e.code == 'unavailable'
          ? '当前手机没有可用的系统语音识别弹窗，请安装/启用 Google 应用或系统语音服务'
          : '系统语音识别弹窗启动失败：${e.message ?? e.code}';
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('系统语音识别弹窗启动失败：$e')),
      );
    } finally {
      _speechFallbackInProgress = false;
      if (mounted) setState(() => _speechListening = false);
    }
  }
